...
/*   Updated: 2024/11/12 15:49:23 by jeportie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
START_TEST(test_ms_execute_external_permission_denied)
{
    ...
    /* Verify the result */
    ck_assert_int_eq(ret, 126); /* Permission denied exit status */
    ck_assert_int_eq(context.exit_status, 126);

    /* Check error message */
    ck_assert_msg(strstr(error_buffer, "minishell: execve error") != NULL,
                "Error message not as expected");
