...
START_TEST(test_ms_init_std_signal_sigint)
{
    pid_t pid = fork();
    if (pid == 0) // Child process
    {
        ...
        exit(g_signal); // Check the global signal variable
    }
    else if (pid > 0) // Parent process
    {
        int status;
...
START_TEST(test_ms_init_std_signal_sigquit)
{
    pid_t pid = fork();
    if (pid == 0) // Child process
    {
...
Suite *ms_init_std_signal_suite(void)
{
    Suite *s;
    TCase *tc_core;

    s = suite_create("ms_init_std_signal");
    tc_core = tcase_create("Core");

    tcase_add_test(tc_core, test_ms_init_std_signal_sigint);
    tcase_add_test(tc_core, test_ms_init_std_signal_sigquit);
