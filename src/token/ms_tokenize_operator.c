...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ms_tokenize_operator.c                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: jeportie <jeportie@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/10/20 17:52:27 by jeportie          #+#    #+#             */
/*   Updated: 2025/01/10 13:48:09 by jeportie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../include/tokenize.h"

static const t_operator_map	*st_get_operator_map(void)
{
...
static void	st_init_op_state(const char **input, t_op_state *op_state)
{
	const t_operator_map	*operator_map;

	ft_memset(op_state, 0, sizeof(t_op_state));
	operator_map = st_get_operator_map();
	op_state->op_map = operator_map;
	op_state->current = (char *)*input;
}

static t_token	*st_allocator(const char **input, t_op_state *v, t_gc *gcl)
{
	t_token_type	type;
	char			*value;
	...
	*input += v->op_len;
	return (ms_create_token(type, value, gcl));
}
...
t_token	*ms_tokenize_operator(const char **input, t_gc *gcl)
{
	...
	while (v.op_map[v.i].op_str)
	{
		...
		if (!ft_strncmp(v.current, v.op_str, v.op_len))
			return (st_allocator(input, &v, gcl));
		v.i++;
	}
	...
	v.value[1] = '\0';
	*input += 1;
	return (ms_create_token(TOKEN_WORD, v.value, gcl));
}
