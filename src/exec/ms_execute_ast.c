...
/*   Updated: 2024/12/17 16:32:55 by jeportie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_proc_manager *manager)
{
	...
	if (node->type == NODE_COMMAND)
		result = (ms_execute_command(&node->data.command, context, manager,
				context->shell->gcl));
	else if (node->type == NODE_PIPE)
		result = (ms_execute_pipeline(&node->data.pipe, context, manager));
	else if (node->type == NODE_AND || node->type == NODE_OR)
		result = (ms_execute_logical(&node->data.logic, context, node->type,
				manager));
	else if (node->type == NODE_SUBSHELL)
		result = (ms_execute_subshell(&node->data.subshell, context, manager));
	else if (node->type == NODE_REDIRECT_IN || node->type == NODE_REDIRECT_OUT
		|| node->type == NODE_REDIRECT_APPEND)
		result = (redirect_switch(node->data.redirect.child, node, context,
				manager));
	else if (node->type == NODE_REDIRECT_HEREDOC)
		result = (redirect_switch(node->data.heredoc.child, node, context,
				manager));
	else
		result = (ms_handle_error("Unsupported node type", -1,
				context->shell->gcl));
	return (result);
}
